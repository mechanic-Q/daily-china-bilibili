...
def verify(date: str) -> dict:
    ...
    if manifest["state"] != "awaiting_user_confirmation":
        raise RuntimeError("样本未停在用户确认门")
    ...
    if (video_stream["width"], video_stream["height"], video_stream["codec_name"]) != (1920, 1080, "h264"):
